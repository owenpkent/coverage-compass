...
The v0.1 read side (notice triage) is built and works locally and deterministically: a PDF, photo, or pasted letter is read on the device, classified against the rule library, and explained in plain language with the deadline, in English and Spanish, with no model at runtime. It passes unit tests, automated accessibility checks, and a production build, and is backed by full documentation (architecture, privacy threat model, accessibility standard, roadmap anchored to the CO timeline, Colorado rules reference, glossary, outreach plan). CCDC and CfA pitches are drafted. The rule library is seeded with five Colorado letter types and eight exemption categories; details are explicitly marked "verify with CCDC" until CCDC reviews.
