...
3. **Done (2026-07-01/02): the engine is ported into the shell.** Schema, tolerant fill helpers, the CDASS packet mapping with embedded I-9, carry-forward import, the `#fill` preview UI, and the opt-in archive are all in `web/src/lib` and live in the demo. The follow-on work is content and capture: the Medicaid schema sections (with CCDC), the first Medicaid form mapping (needs the real form), OCR/barcode capture reuse, and Spanish field labels.
