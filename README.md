...
**Status:** a live demonstration is up at **[coverage-compass-6ky.pages.dev](https://coverage-compass-6ky.pages.dev)**. Drop a PDF or photo of a letter, paste its text, or click one of the built-in fictional example letters, and the app reads it on your device, classifies it against the Colorado rule library, and explains it in plain language with the deadline and next steps, in English or Spanish, in light or dark mode. The demo sits behind a click-through release, with in-app Terms of Use and Privacy Notice pages (drafts, pending legal review). It passes 70 unit tests, automated accessibility checks, and a production build. The rule content and Spanish still need CCDC and native-speaker review before this is more than a demo. The write side (document capture and exact PDF form-fill) is already a working proof of concept in a sibling project, [CDASS Enroll](https://github.com/owenpkent/cdass-enroll); Coverage Compass is re-architected around it. First public version targeted August 2026, before the state begins mailing notification letters.
